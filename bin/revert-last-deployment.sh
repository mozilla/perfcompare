...
UPSTREAM=$(git remote -v | grep -E 'mozilla/perfcompare' | head -1 | awk '{ print $1 }')
if [ -z "$UPSTREAM" ] ; then
  echo 1>&2 "Error: couldn't find the upstream remote. Is it well configured?"
  echo 1>&2 "We're looking for mozilla/perfcompare."
  exit 1
fi
...
echo ">>> Running tests"
echo_and_exec_cmd npm ci
echo_and_exec_cmd npm run test
echo_and_exec_cmd npm run lint

echo ">>> Force-pushing to $UPSTREAM's production branch. Note you'll need to disable
branch protection for this to succeed."
echo "Go to https://github.com/mozilla/perfcompare/settings/branches and enable force pushing."
printf "Hit enter when you are ready."
read -r enter
echo_and_exec_cmd git push -f --no-verify "$UPSTREAM" HEAD:production

echo "Please re-enable now the force push protection on https://github.com/mozilla/perfcompare/settings/branches"
printf "Hit enter when you are ready."
