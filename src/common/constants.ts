...
export const tooltipModeDelta =
  'Largest peak shift between the Base and New distributions, expressed as a percentage of the base peak location. Peaks are detected by KDE mode analysis and paired with the modality matcher; the matched pair with the largest absolute shift is shown. Positive values mean the new peak shifted higher. Values prefixed with "~" are the backend\'s median diff percentage shown as an approximation — click the column header to compute the real Mode Δ across all rows. "No modes" means the value could not be computed: too few runs (fewer than 2 per side), KDE could not find any peaks on one or both sides, or no base peak could be paired with a new peak (a path appeared or disappeared entirely).';
