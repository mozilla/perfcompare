...
describe('useDocsURL', () => {
  ...
  it('should return the correct URL for a supported perfdocs framework', () => {
    ...
    expect(result.current.docsURL).toBe('https://firefox-source-docs.mozilla.org/testing/perfdocs/talos.html#testsuite');
  ...
  it('should return the correct URL for "devtools" framwork', () => {
    ...
    expect(result.current.docsURL).toBe('https://firefox-source-docs.mozilla.org/devtools/tests/performance-tests-overview.html#anothertestsuite');
    expect(result.current.isLinkSupported).toBe(true);
  });
  ...
  it('should handle suite names with special characters correctly', () => {
    const { result } = renderHook(() => useDocsURL('Suite With:Special.Characters', 12));

    expect(result.current.docsURL).toBe('https://firefox-source-docs.mozilla.org/devtools/tests/performance-tests-overview.html#suite-with-special-characters');
    expect(result.current.isLinkSupported).toBe(true);
  });
});
